...
//该类实现了ChannelBuffer接口，是通道缓存的抽象类，它
// 实现了ChannelBuffer所有方法，但是它实现的方法都是需要被重写的方法，
// 具体的实现都是需要子类来实现。现在我们来恶补一下这个通道缓存的原理，
// 当然这个原理跟netty的ByteBuf原理是分不开的。AbstractChannelBuffer维护了
// 两个索引，一个用于读取，另一个用于写入当你从通道缓存中读取时，
// readerIndex将会被递增已经被读取的字节数，
...
public abstract class AbstractChannelBuffer implements ChannelBuffer {
    ...
    //标记读索引和标记写索引是为了做备份回滚，当对缓冲区进行读写操作时，
    // 可能需要对之前的操作进行回滚，
